...
평준화가 빠지면 컴프레서의 makeup gain 도 같이 사라져서 그만큼 조용해진다.
그 몫을 **고정 +15 dB** 로 되돌려 보낸다 (`audio_dsp.RTP_BOOST_DB`).

볼륨 슬라이더는 폰 송출 중에는 아무 영향이 없다. 볼륨은 폰에서 조절하는 것이고,
PC 쪽 슬라이더까지 끼어들면 어느 쪽이 먹은 것인지 알 수 없어진다.

게인을 올리면 큰 소리에서 0 dBFS 를 넘어 깨지므로 뒤에 리미터를 하나 둔다.
평준화용이 아니라 피크만 막는 안전장치라 -0.5 dBFS 를 넘지 않는 구간에서는 아무 일도 하지 않는다.

실측 (1kHz, 게인 +15 dB, RTP 로 나가는 소리를 직접 받아서 측정):

| 구간 | 게인 없음 | +15 dB |
|---|---|---|
| 조용한 대사 | -34 dBFS | **-19 dBFS** (+15 그대로) |
| 시끄러운 액션 | -10 dBFS | -4 dBFS (리미터가 나머지 흡수) |
| 전체 피크 | -7.3 dBFS | **-0.50 dBFS**, 클리핑 0 샘플 |
